...
# 9. Cell Tower Performance Comparison
tower_stats = df.groupby('PCI').agg({
    'RSRP': 'mean',
    'SINR': 'mean',
    'RSRQ': 'mean',
    'PCI': 'count'
}).rename(columns={'PCI': 'Sample_Count'}).reset_index()
...
fig_towers.add_trace(
    go.Bar(x=tower_stats['PCI'].astype(str), y=tower_stats['Sample_Count'], 
           name='Samples', marker_color='steelblue'),
    row=1, col=1
)

fig_towers.add_trace(
    go.Scatter(x=tower_stats['PCI'].astype(str), y=tower_stats['RSRP'],
               mode='markers+lines', name='Avg RSRP', 
               marker=dict(size=10, color='blue')),
    row=1, col=2
)
fig_towers.add_trace(
    go.Scatter(x=tower_stats['PCI'].astype(str), y=tower_stats['SINR'],
               mode='markers+lines', name='Avg SINR',
               marker=dict(size=10, color='green'), yaxis='y2'),
    row=1, col=2
)

fig_towers.update_xaxes(title_text="Tower PCI", row=1, col=1)
fig_towers.update_xaxes(title_text="Tower PCI", row=1, col=2)
...
fig_towers.update_layout(height=450, showlegend=True, 
                        title_text="Cell Tower Performance Analysis",
...
def update_dashboard(rsrp_range, sinr_range, quality_filter, ts_metric, custom_x, custom_y, custom_color):
    ...
    fig1 = px.scatter_geo(df_filtered, lat='Latitude', lon='Longitude', color='RSRP', 
                         ...
                         hover_data=['Time', 'PCI', 'RSRP', 'SINR'], height=500)
    ...
    fig3.add_trace(go.Histogram(x=df_filtered['RSRP'], name='RSRP', marker_color='blue', nbinsx=30), row=1, col=1)
    fig3.add_vline(x=-80, line_dash="dash", line_color="green", row=1, col=1)
    fig3.add_vline(x=-90, line_dash="dash", line_color="red", row=1, col=1)
    
    fig3.add_trace(go.Histogram(x=df_filtered['RSRQ'], name='RSRQ', marker_color='orange', nbinsx=30), row=1, col=2)
    fig3.add_trace(go.Histogram(x=df_filtered['SINR'], name='SINR', marker_color='green', nbinsx=30), row=2, col=1)
    
    class_counts = df_filtered['Connection_Class'].value_counts()
    fig3.add_trace(go.Bar(x=class_counts.index, y=class_counts.values, 
                         marker_color=['green', 'yellow', 'red']), row=2, col=2)
    
    fig3.update_layout(height=600, showlegend=False, title_text="Signal Quality Distributions")
    
    # 4. Correlation Heatmap
    corr_df = df_filtered[['RSRP', 'RSRQ', 'SINR', 'Elevation', 'Speed_kmh']].corr()
    fig4 = go.Figure(data=go.Heatmap(z=corr_df.values, x=corr_df.columns, y=corr_df.columns,
                                     colorscale='RdBu', zmid=0, text=corr_df.values.round(2),
                                     texttemplate='%{text}', textfont={"size": 12}))
    fig4.update_layout(title='Correlation Matrix', height=450)
    
    # 5. Quality Zones Scatter
    sample_data = df_filtered.sample(min(5000, len(df_filtered)))
    fig5 = px.scatter(sample_data, x='RSRP', y='RSRQ', color='Connection_Class',
                     color_discrete_map={'Excellent':'green', 'Moderate':'yellow', 'Poor':'red'},
                     title='Signal Quality Zones (RSRP vs RSRQ)', opacity=0.6, height=450)
    fig5.add_hline(y=-10, line_dash="dash", line_color="gray")
    fig5.add_vline(x=-85, line_dash="dash", line_color="gray")
    
    # 6. Time Series
    if ts_metric == 'all':
        fig6 = make_subplots(rows=3, cols=1, shared_xaxes=True,
                            subplot_titles=('RSRP', 'SINR', 'Speed'),
                            vertical_spacing=0.08)
        fig6.add_trace(go.Scatter(x=df_filtered['Time'], y=df_filtered['RSRP'], 
                                 name='RSRP', line=dict(color='blue')), row=1, col=1)
        fig6.add_trace(go.Scatter(x=df_filtered['Time'], y=df_filtered['SINR'], 
                                 name='SINR', line=dict(color='green')), row=2, col=1)
        fig6.add_trace(go.Scatter(x=df_filtered['Time'], y=df_filtered['Speed_kmh'], 
                                 name='Speed', line=dict(color='red')), row=3, col=1)
        fig6.update_layout(height=700, title_text="Metrics Over Time")
    else:
        fig6 = px.line(df_filtered, x='Time', y=ts_metric, title=f'{ts_metric} Over Time')
    ...
    fig7.add_trace(go.Scatter(x=df_filtered['Time'], y=df_filtered['RSRP'],
                             mode='lines', name='RSRP', line=dict(color='gray', width=1),
                             yaxis='y', opacity=0.5))
    fig7.add_trace(go.Scatter(x=df_filtered['Time'], y=df_filtered['PCI'],
                             mode='lines', name='PCI', line=dict(shape='hv'), yaxis='y2'))
    fig7.update_layout(title='RSRP vs Tower Switching (PCI)',
                      yaxis=dict(title='RSRP (dBm)'), 
                      yaxis2=dict(title='PCI', overlaying='y', side='right'),
                      height=500)
    
    # 8. Tower Performance
    tower_stats = df_filtered.groupby('PCI').agg({
        'RSRP': 'mean', 'SINR': 'mean', 'PCI': 'count'
    }).rename(columns={'PCI': 'Count'}).reset_index().sort_values('Count', ascending=False).head(10)
    
    fig8 = make_subplots(rows=1, cols=2, 
                        subplot_titles=('Top Towers by Usage', 'Avg Signal Quality'),
                        specs=[[{"type": "bar"}, {"type": "scatter"}]])
    
    fig8.add_trace(go.Bar(x=tower_stats['PCI'].astype(str), y=tower_stats['Count'],
                         marker_color='steelblue'), row=1, col=1)
    fig8.add_trace(go.Scatter(x=tower_stats['PCI'].astype(str), y=tower_stats['RSRP'],
                             mode='markers+lines', name='RSRP', marker=dict(size=10)), row=1, col=2)
    fig8.update_layout(height=450, title_text="Cell Tower Performance")
    ...
    for col in ['Excellent', 'Moderate', 'Poor']:
        if col in quality_pivot.columns:
            color = {'Excellent': 'green', 'Moderate': 'yellow', 'Poor': 'red'}[col]
            fig9.add_trace(go.Bar(x=quality_pivot.index, y=quality_pivot[col], 
                                 name=col, marker_color=color))
    fig9.update_layout(barmode='stack', title='Connection Quality by Hour', 
                      xaxis_title='Hour', yaxis_title='Count', height=450)
    
    # 10. Signal Density Heatmap
    fig10 = go.Figure(go.Histogram2d(x=df_filtered['RSRP'], y=df_filtered['SINR'],
                                     colorscale='Viridis', nbinsx=50, nbinsy=50))
    fig10.update_layout(title='Signal Quality Density (RSRP vs SINR)',
                       xaxis_title='RSRP (dBm)', yaxis_title='SINR (dB)', height=450)
    
    # 11. Speed Impact
    fig11 = px.scatter(df_filtered, x='Speed_kmh', y='SINR', color='RSRP',
                      trendline="ols", title='Speed Impact on Signal Quality',
                      labels={'Speed_kmh': 'Speed (km/h)', 'SINR': 'SINR (dB)'}, height=450)
    
    # 12. Elevation Impact
    fig12 = px.scatter(df_filtered, x='Elevation', y='RSRP', color='PCI',
                      title='Elevation Impact on Signal Strength',
    ...
    fig13 = px.scatter(sample_custom, x=custom_x, y=custom_y, color=custom_color,
    ...
    return (filter_text, fig1, fig2, fig3, fig4, fig5, fig6, fig7, 
